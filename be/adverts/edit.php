...
if ($_SERVER['REQUEST_METHOD']=='GET') {
	...
	$pid = (int)$array['in_data']['position_id'];
	$con['controls']['ad_image']['control']['parameters']['sizes']['img'] = $GLOBALS['AdsPositionsSize'][$pid];

		
	} else {
		$array['in_data']['ad_type_id'] = 1;
	}
}
else {
	$array=$_POST;

	$pid = (int)$_POST['in_data']['position_id'];
	$con['controls']['ad_image']['control']['parameters']['sizes']['img'] = $GLOBALS['AdsPositionsSize'][$pid];
}
...
*/
			...
			$errors+=ControlValues::processManagedFiles($n_id,$_FILES,$con['controls'],true,true);
