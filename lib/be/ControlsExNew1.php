...
class DataGridNew
{
    ...
    function createFromArray($array, $renderEvents = false)
    {
      ...
      //  if (isset($array['hasExcelExport']) && $array['hasExcelExport'])
       ...
       // if (isset($array['hasPrint']) && $array['hasPrint'])
        ...
        //}
        $this->excel_options=array('skip_columns'=>array("id"));
    ...
    function renderHead(&$dom)
    {
        ...
        {
            $fo = array_keys($this->currentorder);
            $fo = strtolower($fo[0]);
        }
            ...
            foreach ($thead_a as $v)
            {
                ...
                if ($v->hasAttribute('order'))
                {
                    ...
                    if (!empty($src))
                    {
                        ...
                        $parent=$v->parentNode;
                        if($parent) {
                        	$parent->setAttribute("class","header_ord");
                        }
                    }
                }
            }
    ...
    private function processChildren(&$node, &$dom, $index)
    {
        ...
        foreach ($node->childNodes as $ch)
        {
            if ($ch instanceof DOMText)
            {
            	...
            	//$ch->nodeValue=$this->replaceSpecialConstants($ch->nodeValue,$index,"");
            	//$n=new DOMText($this->replaceSpecialConstants($ch->nodeValue,$index,""));
            	//$ch->parentNode->appendChild($n);
            	//$ch->parentNode->replaceChild($n,$ch);
...
EOD;
        ...
        if($this->hasPrint) {
        	$xls_buttons[]=<<<EOD
        	<input onclick="getForm(this).target='_blank';document.getElementById('{$this->control_id}bt_print').value='1';getForm(this).submit();getForm(this).target='';document.getElementById('{$this->control_id}bt_print').value='';" type="button" value="Print" />
        	<input type="hidden" name="{$this->control_id}bt_print" value="" />
EOD;
        }
        if($this->hasExcelExport) {
        	$xls_buttons[]="<input type=\"submit\" name=\"{$this->control_id}_xls\" value=\"Excel\" />";
        }
        
        $xls="";
        if(!empty($xls_buttons)) {
        	$xls="<div style='width:100%;' align=\"right\">".implode("&nbsp;",$xls_buttons)."</div>";
        }
        //else {
        //	$xls="";
        //}
