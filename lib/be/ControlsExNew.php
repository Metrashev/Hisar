<?php
define('IMAGE_ORDER_UP',BE_IMG_DIR.'sorted_up_1.gif');
define('IMAGE_ORDER_DOWN',BE_IMG_DIR.'sorted_down_1.gif');

define('IMAGE_ORDER_UP_FIRST',BE_IMG_DIR.'sorted_up.gif');
define('IMAGE_ORDER_DOWN_FIRST',BE_IMG_DIR.'sorted_down.gif');
...
class CClientFileUploader extends CClientControlRepearer
{
    ...
    function renderUploadedFiles($template = '', $useDelete = true)
    {
        ...
        if (empty($template))
        {
            ...
            $template = "<tr>
			<td><a href='".BE_DIR."fileProcessor.php?id=_#ID#_' target='_blank'>_#NAME#_</a></td>
			{$str_delete}
			</tr>";
            if ($useDelete)
            {
...
class ExcelExporter
{
    ...
    {
    	ExcelExporter::exportHTML($array,$col_offset,$title,$filename);
    	return;
    ...
    static function exportHTML($array, $col_offset = 0, $title = '', $filename = '') {
    	$l=ob_get_level();
    	while ($l>=0) {
    		ob_get_clean();
    		$l--;
        ...
        foreach ($array as $k => $v)
        {
            ...
            foreach ($v['rows'] as $r_row)
            {
                ...
                //	$rv=iconv("utf-8","windows-1251",$rv);
					echo ereg("^[0-9]+\.[0-9]{2}$", $rv) ? '<td class="n">': "<td>";
...
class DataGridNew
{
    ...
    function createFromArray($array, $renderEvents = false)
    {
        ...
        if (isset($array['OrderCallback']))
        {
            $this->OrderCallback =$array["OrderCallback"];
        }
        
        if (isset($dt['union']) && !empty($dt['union']))
        {
            $this->DataSource->Union = $dt['union'];
        }
        ...
        if(isset($array["is_php_table"])&&$array["is_php_table"]) {
        	$this->IsPhpTable=true;
        }
    ...
    static function getImageStatic($field, &$fo,$currentorder)
    {
        $img_src = '';
    ...
    static function getFieldOrderString($fld,$Label,$currentorder,$control_id) {
    	static $fo_a=null;
    	$fo="";
    	if(is_null($fo_a)) {
	    	if (is_array($currentorder))
	        {
	            $fo_a = array_keys($currentorder);
        ...
        if(!is_null($sort_index)) {
        	...
        	$sup="<sup>{$sort_index}</sup>";
        	
        	if($sort_index>1) {
        		$ho=2;
        	}
        }
    	$src=DataGridNew::getImageStatic($fld,$fo,$currentorder);
    	if(!empty($src)) {
    		$img=<<<EOD
    		<img src="{$src}" />
EOD;
    	...
    	return <<<EOD
    	<a onclick="_table_setOrder(event,this,&quot;{$control_id}[neworder]&quot;,&quot;{$fld}&quot;)" href="#">{$Label}{$img}{$sup}</a>
EOD;
    }
    ...
    public function readOrder($new_order=null)
    {
        ...
        if(!empty($this->OrderCallback)) {
        	call_user_func($this->OrderCallback,array($this->currentorder,$this));
        }        
    }
        ...
        if ($thead_a && $thead_a->length)
        {
            foreach ($thead_a as $v)
            {
                ...
                if ($v->hasAttribute('order'))
                {
                    ...
                    $v->setAttribute("onclick", '_table_setOrder(event,this,"' . $this->control_id .
        ...
        foreach ($node->childNodes as $ch)
        {
            ...
            if (!empty($fields['userfunc']))
            {
            	$value = (string )call_user_func(array($fields['class'], $fields['userfunc']),
                    array($this->DataSource->Rows[$index][$fields['field_name']], $index, $this, $ch,$dom,$fields['field_name'],$cell_counter));
            }
            ...
            if (((string )$value) !== "")
            {
            	if($encode_chars) {
            		...
            		$tn=(string)$ch->tagName;
            		$tn=strtolower($tn);
            		switch ($tn)  {
            			case "img": {
            				//ne se pokazva stoinostta
            				break;
            			}
            			default: {
    ...
    private function processChildrenExcel(&$node, &$dom, $index,$row)
    {
        ...
        foreach ($node->childNodes as $ch)
        {
            ...
            if (isset($row[$fields['field_name']]))
            {
                if (((string )$ch->nodeValue) !== "")
                {
                	//$value = $this->replaceSpecialConstants($ch->nodeValue, $index, $row[$fields['field_name']]);
                	$rep_index = $this->page_size * $this->currentPage + $index;
			        $arr = array(
						'_#INDEX#_' => $escate ? mysql_real_escape_string($rep_index):$rep_index,
			            '_#VAL#_' => $escate ? mysql_real_escape_string($value):$value, 
						'_#UNIQUE#_' =>$row[$this->data_unique_field],
			            '_#CONTROL#_' => $this->control_id,
			        	'_#ID#_'=> $row['id'],
			        );
			
			        $value= str_replace(array_keys($arr), $arr, (string)$ch->nodeValue);
                }
                else
                {
            ...
            {
            	$value = (string )call_user_func(array($fields['class'], $fields['userfunc']),
                    array($row[$fields['field_name']], $index, $this, $ch,$dom));
            }
       		 if (!empty($fields["multi_sql"]))
            {
            	if(!empty($row[$fields['field_name']])) {
	                $db = getdb();
			        ...
			        $sql= str_replace(array_keys($arr), $arr, $fields["multi_sql"]);
	
	               	//$sql = $this->replaceSpecialConstants($fields["multi_sql"], $index, $row[$fields['field_name']]);
	                try {
	                $value = $db->getcol($sql);
	                }
	                catch(Exception $e) {
            ...
            if (!empty($fields["sql"]))
            {
                ...
                $rep_index = $this->page_size * $this->currentPage + $index;
			        ...
			        $sql= str_replace(array_keys($arr), $arr, $fields["sql"]);
               // $sql = $this->replaceSpecialConstants($fields["sql"], $index, $this->DataSource->
                //    Rows[$index][$fields['field_name']]);
                $value = (string )$db->getone($sql);
            }
            if (!empty($fields['arrayname']))
            {
                if (isset($GLOBALS[$fields['arrayname']]))
                {
                    $value = (string )$GLOBALS[$fields['arrayname']][$row[$fields['field_name']]];
                }
                else
                {
                    @$arr = unserialize($fields['arrayname']);
                    $value = (string )$arr[$row[$fields['field_name']]];
                }
            }
			 if (!empty($fields['multi_arrayname']))
            {
            	if (isset($GLOBALS[$fields['multi_arrayname']]))
                {
                	$values=explode(',',$row[$fields['field_name']]);
                	$value=array();
                	foreach ($values as $vv) {
                		if(isset($GLOBALS[$fields['multi_arrayname']][$vv])) {
                			$value[]=$GLOBALS[$fields['multi_arrayname']][$vv];
            ...
            //$this->replaceAttributes($ch, $index, (string )$row[$fields['field_name']]);
            if ($ch->hasAttribute("name"))
            {
                $this->setValue($ch);
            }
            ...
            if ($ch->hasChildNodes())
            {
                $this->processChildrenExcel($ch, $dom, $index,$row);
            }
        }
    }
    ...
    function renderBody(&$dom)
    {
        ...
        if ($tbody && $tbody->length)
        {
            foreach ($tbody as $v)
            {
               ...
               /* if($this->is_excel_render) {
                	foreach ($this->DataSource->Rows as $i=>$row) {
                		echo "<tr>";
                		if ($v->hasChildNodes())
		                {
		                    //try {
		                    $this->processChildrenExcel($v, $dom,$i,$row);
		                    //}
		                    //catch(Exception $e) {
		                    //	var_dump($e);
		                    //}
    ...
    function renderXls(&$dom) {
	     ...
	     $this->DataSource->buildquery(null);
	     $this->renderHead($dom);
	     $this->renderBody($dom);
/*
	     $this->DataSource->buildquery(null,true);
...
EOD;
    	...
    	$c_row = 0;
	     ...
	     echo "<tr>";
            foreach ($this->matrix_header as $hv)
            {
            //	$hv=iconv("utf-8","windows-1251",$hv);
            	echo "<th>";
            	echo htmlspecialchars($hv);
...
EOD;
    ...
    function renderPhpTable() {
        ...
        $order_script=$this->getScriptText();
        $hd="<input type='hidden' name='{$this->control_id}[neworder]' id='{$this->control_id}_neworder' value=\"".htmlspecialchars($this->getOutputOrder())."\" />";
        
        $butt=$this->gettablebuttons(); 
		        
        ob_start();
        include($this->template);
        return DataGridNew::getTableHeaderLine($butt).ob_get_clean().$order_script.$hd.DataGridNew::getTableFooterLine($butt);
        
    }
    
    public function getScriptText($add_script_tag=true) {
...
");
    	...
    	$b=array();
    	if($this->hasPrint) {
    		$b[]=DataGridNew::getPrintButton($this->control_id);
    	}
    	if($this->hasExcelExport) {
    		$b[]=DataGridNew::getXlsButton($this->control_id);
    	}
    	return $b;
    }
    
    static function getTableHeaderLine($buttons) {
    	if(!empty($buttons)) {
    		$xls="<td align=\"right\" style='padding-right:5px;'>".implode("&nbsp;&nbsp;",$buttons)."</td>";
    	}
    	return <<<EOD
<div class="listTableHeader">
 <div class="listTableHeaderBody">
...
EOD;
    ...
    static function getTableFooterLine($buttons) {
    	if(!empty($buttons)) {
    		$xls="<td align=\"right\" style='padding-right:5px;'>".implode("&nbsp;&nbsp;",$buttons)."</td>";
    	}
    	return <<<EOD
<div class="listTableFooter">
 <div class="listTableFooterBody">
<table class="listTable_f" cellpadding="0" cellspacing="0" width="100%">
<tr>	
	{$xls}
</tr>
</table>
...
EOD;
        ...
        if(!empty($xls_buttons)) {
        	...
        	$xls="<td align=\"right\" style='padding-right:5px;'>".implode("<img align='absmiddle' src='".BE_IMG_DIR."design/v_separator.png' />&nbsp;&nbsp;",$xls_buttons)."</td>";
        }
        //else {
        //	$xls="";
        //}
...
function _table_setOrder(event,obj,element,new_order) {
