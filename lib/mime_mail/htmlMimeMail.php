...
* Last Modified..: $Date: 2002/07/17 11:28:57 $
...
*/
	...
	{
		$this->headers['Subject'] = substr($subject,0,255);
	}
...
*/
	function _findHtmlImages($images_dir)
	{
		...
		preg_match_all('/(background=|url|src=)(?:"|\'|\()([^"\'\)]+)(?:"|\'|\))/Ui', $this->html, $images);
		

		for ($i=0; $i<count($images[2]); $i++) {
			if (file_exists($images_dir . $images[2][$i])) {
				$html_images[] = $images[2][$i];
				$this->html = str_replace($images[2][$i], basename($images[2][$i]), $this->html);
			}
		}
...
*/
	/*function _encodeHeader($input, $charset = 'ISO-8859-1')
	{
	 ...
	 function _encodeHeader($input, $charset = 'UTF-8')
     {
          ...
          //preg_match_all('/([^<>,]*[\x00-\x1f\x80-\xff]+[^<>,]*)/', $input, $matches);
          preg_match_all('/([^<>,]*[\x80-\xff]+[^<>,]*)/', $input, $matches);
          foreach ($matches[1] as $value) {
	 ...
	 *
	 */
	function getEncodedMail($recipients, $type = 'mail') {
		if (!defined('CRLF')) {
		...
		if (!empty($this->return_path)) {
			return array(
				...
				'additional_params'=>'-f' . $this->return_path,
			);			
		} else {
			return array(
				'to'=>$to,
	...
	function send($recipients, $type = 'mail')
	{
		...
		switch ($type) {
			case 'mail':
				...
				if (!empty($this->return_path)) {
					return mail($to, $subject, $this->output, implode(CRLF, $headers), '-f' . $this->return_path);
				} else {
					return mail($to, $subject, $this->output, implode(CRLF, $headers));
				}
				break;
